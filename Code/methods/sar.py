...
class SAR(nn.Module):
    ...
    """
    def __init__(self, model, optimizer,optimizer2, steps=1, episodic=False, margin_e0=0.4*math.log(1000), reset_constant_em=0.005):
        super().__init__()
        self.model = model
...
def forward_and_adapt_sar(x, model, optimizer, optimizer2, margin, reset_constant, ema):
    ...
    #plot_tsne(features,filter_ids_1[0],'Before')
    if len(x[filter_ids_1])!=0:
    ...
    # filtering reliable samples/gradients for further adaptation; first time forward
    entropys = softmax_entropy(outputs)
    filter=filter_ids_1[0][filter_ids_2[0]]
    entropys=entropys[filter]
    loss=entropys.mean(0)
    loss.backward()
    optimizer.second_step(zero_grad=True)
    optimizer.zero_grad()
